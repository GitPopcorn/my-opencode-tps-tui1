...
SetLocal
@REM ================================================================
@REM  重建 my-opencode-tps-tui1 的缓存目录链接 (JUNCTION)
@REM
...
@REM      于是插件真身留在本地仓库，tui.jsonc 里只写包名
@REM
@REM  要点
...
SET "PLUGIN_DIR_REL=.."
SET "PLUGIN_ENTRY=tui.tsx"
SET "PLUGIN_CONFIG_FILE=tui.jsonc"
...
@REM STEP 哨兵 4：TUI 插件专属 —— 本地 @opentui 会与宿主抢注册环境变量
@REM WARN 命中时插件会被静默丢弃，因此直接中止
IF EXIST "%PLUGIN_DIR%\node_modules\@opentui" (
...
:USAGE
ECHO 重建 my-opencode-tps-tui1 的缓存目录链接 ^(JUNCTION^)
ECHO.
ECHO 用法：
